...
const AccomodationForm = () => {
  ...
  const uploadPhoto = (e) => {
    ...
    axios.post('http://localhost:5001/api/places/uploadphotos', data, {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post('http://localhost:5001/api/places/addplace', placeData, {
        headers: {
          ...
          <div className='mt-2 grid gap-2 grid-cols-3 md:grid-col-4 lg:grid-cols-6'>
            {addedPhotos.length > 0 && addedPhotos.map((photo) => {
              return <div className='h-32 flex relative' key={photo}>
                <img className='rounded-2xl w-full object-cover' src={`http://localhost:5001/uploads/${photo}`} alt="" />
